#import necessary libaries
import tkinter as tk
from tkinter import *
import ttkbootstrap as ttk
from ttkbootstrap.constants import * 
from ttkbootstrap import Label

focusBorderImageData = '''
    
'''

borderImageData = '''
    
'''

#create window
root = ttk.Window(themename="darkly")
#resize window
root.geometry("800x500")

#define style
style = ttk.Style()
style.theme_use("default)")

#create rounded corner style
style.configure("Rounded Edge",
                
)


my_frame = Frame(root)

canvas = Canvas(my_frame, width=500, height=400, background='gray75')

scrollbar = ttk.Scrollbar(root, orient='vertical')

class bookrecomendation(): 
    def __init__(self):
        self.bookrecomendations = ttk.Label(root,
                                text = (name_entry.get()),
                                font=("Arial", 20, "bold"),
                                background = "white",
                                borderwidth = 5,
                                width = 10,
                                padding = 10,
                                foreground = "blue")
        
        

    def placement(self, x = 450, y = 50):
        self.bookrecomendations.place(x=x, y=y)


def on_hover(event):
    print("Hello")
    global book1
    book1 = bookrecomendation()
    book1.placement()


def on_default(event):
    print("none")
    book1.placement(x=800,y = 400)


name_entry = ttk.Entry(root,textvariable = ttk.StringVar, font=('calibre',10,'normal'))


title = ttk.Label(root,
                  text = "Book Recomendation",    
                font=("Arial", 20, "bold"),
                )


btn = Button(root, 
             text="book title", 
             font = ("", 10), bg = "#1C6758",
             width = 15,
             activebackground = "#1C6758",
             cursor = "hand2")


def bookrecomendations(name):
    None

def main():
    title.place(x=0, y=0)

    btn.bind('<Enter>', on_hover)
    btn.bind('<Leave>', on_default)
    btn.place(x=600, y = 0)

    name_entry.place(x = 0, y = 60)

    scrollbar.place(x=780,y=0)

    root.mainloop()

if __name__ == '__main__':
    main()